...
func _on_touch_damage_area_area_entered(area: Area2D) -> void:
	...
	var damaged := apply_damage((area as Bullet).damage if area is Bullet else DEFAULT_BULLET_DAMAGE)
	if damaged:
		bullet.queue_free()
...
func _try_drop_coin() -> void:
	if config == null:
		return
	if config.coin_value <= 0:
		return
	if random_generator.randf() > clampf(config.coin_drop_chance, 0.0, 1.0):
		return

	var random_offset := Vector2(
		random_generator.randf_range(-4.0, 4.0),
		random_generator.randf_range(-4.0, 4.0)
	)
	call_deferred("_spawn_dropped_coin", config.coin_value, global_position + random_offset)
...
func _spawn_dropped_coin(coin_value: int, spawn_position: Vector2) -> void:
	var drop_parent := get_parent()
	if drop_parent == null:
		return
